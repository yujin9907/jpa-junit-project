...
public class FilterConfig {
    ...
    // 최초 서버 실행시 실행
    @Bean // ioc에 등록해야 됨
    public FilterRegistrationBean<HelloFilter> jwtAuthenticationFilterResister() {
        log.debug("디버그 : 인증필터실행");
        FilterRegistrationBean<HelloFilter> bean = new FilterRegistrationBean<>(new HelloFilter()); // 필터 등록을 위한 생성
